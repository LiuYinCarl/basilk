...
- **Panic Safety**: `init_terminal` installs a panic hook that calls `restore_terminal()` before delegating to the default hook, so a panic never strands the terminal in raw mode / alternate screen. Keep this hook in place when touching terminal setup.
